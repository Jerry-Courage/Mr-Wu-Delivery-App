...
export async function getRecommendations(menuItems, recentOrders, timeOfDay) {
...
export async function getOrderETA(status, createdAt, itemCount) {
...
export async function getKitchenSummary(orders) {
    if (orders.length === 0)
        return "No active orders right now. Enjoy the quiet!";
    const filtered = orders.filter(o => o && !["delivered", "cancelled"].includes(o.status)).slice(0, 10);
    if (filtered.length === 0)
        return "No active orders to report. Great time to prep for the next rush!";
...
export async function searchMenu(query, menuItems) {
    const menuText = menuItems
...
export async function getSupportResponse(userQuery, history = []) {
